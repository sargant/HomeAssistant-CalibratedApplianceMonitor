...
DRY_MAX_W = 1300.0
DRY_CONFIRM = 10
# A recorded run contained a ~101-second telemetry hole while the retained
# power value was below START_W. This plug is change-driven, so silence is not
# itself unavailability; require two minutes of quiet so that hole cannot finish
# a washing cycle before contradictory activity returns.
WET_FINISH_CONFIRM = 2 * 60
DRY_FINISH_CONFIRM = 10
POWER_UNAVAILABLE_ABANDON = 10 * 60
...
class HooverHBDOS695TAMCE80Monitor(ApplianceMonitor):
    ...
    continuous <10 W quiet: 120 seconds while washing and 10 seconds once drying
    has been seen. The Matter source is change-driven, so an unchanged numeric
    value remains valid until Home Assistant explicitly reports unknown or
    unavailable. Explicit source unavailability breaks continuity-sensitive
    debounce windows; an active observation that stays unavailable for ten
    minutes is abandoned without recording a completion. The official finish is
    backdated to the first quiet sample. Finished remains visible for at least
    one minute and at most ten.
    """
    ...
    async def async_setup(self) -> None:
        ...
        if self.state == FINISHED and not self.candidate_started_at:
        ...
        if self.running:
            ...
            self._schedule_runtime(
                "power_unavailable",
                POWER_UNAVAILABLE_ABANDON,
                self._power_unavailable_timeout,
            )
    ...
    def _power_changed(self, event: Event[EventStateChangedData]) -> None:
        ...
        if new is None:
            if was_available:
                ...
                self._cancel_finish_candidate()
                self._changed()
            if self.running and "power_unavailable" not in self.timers:
                self._schedule_runtime(
                    "power_unavailable",
                    POWER_UNAVAILABLE_ABANDON,
                    self._power_unavailable_timeout,
        ...
        self._cancel_runtime("power_unavailable")
    ...
    @callback
    def _power_unavailable_timeout(self, _now: datetime) -> None:
        if self.available or not self.running:
            return
